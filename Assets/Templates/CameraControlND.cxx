...
public class CameraControlND : BasicCamera<D> {
    ...
    [System.NonSerialized] public QTYPE m1 = QTYPE.identity;
    ...
    private VECTOR smoothGravityDirection = (VECTOR)Vector3.up;
    private VECTOR intermediateGravityDirection = (VECTOR)Vector3.up;
    protected MATRIX gravityMatrix = MATRIX.identity;
    ...
    private QTYPE vrM1Unfiltered = QTYPE.identity;
    ...
    public override void Reset() {
        ...
        volumeSmooth = 0.0f;
        volumeStartYZ = 0.0f;
        smoothGravityDirection = (VECTOR)Vector3.up;
        intermediateGravityDirection = (VECTOR)Vector3.up;
        gravityMatrix = MATRIX.identity;
        fastGravity = false;
        zoom = 1.0f;
        targetZoom = 1.0f;
    }
    ...
    public VECTOR HandleMoving() {
        ...
        if (accel != VECTOR.zero) {
            ...
            if (useGravity) {
                accel -= smoothGravityDirection * VECTOR.Dot(smoothGravityDirection, accel);
            }
    ...
    protected virtual void Update() {
        ...
        float angle = Transform<D>.Angle(gravityDirection, smoothGravityDirection);
        if (fastGravity || angle > 60.0f) {
            intermediateGravityDirection = gravityDirection;
        } else {
            intermediateGravityDirection = Transform<D>.RotateTowards(intermediateGravityDirection, gravityDirection, maxAngle);
            angle = Transform<D>.Angle(intermediateGravityDirection, smoothGravityDirection);
        }
        smoothGravityDirection = Transform<D>.RotateTowards(intermediateGravityDirection, smoothGravityDirection, gravitySmooth * angle);
        gravityMatrix = Transform<D>.OrthoIterate(Transform<D>.FromToRotation(gravityMatrix.GetColumn(1), smoothGravityDirection) * gravityMatrix);
    ...
    protected virtual void FixedUpdate() {
        ...
        if (!locked && oddFrame) {
            ...
            colliderPosition<D> = UpdatePhysics(colliderPosition<D>, 2.0f * Time.fixedDeltaTime);
        }
    }
    ...
    public override VECTOR camPosition<D> {
        get {
            VECTOR result = position<D>;
    ...
    public VECTOR colliderPosition<D> {
        get {
            VECTOR result = position<D>;
    ...
    public MATRIX CreateCamMatrix(QTYPE m1Rot, float yz) {
        //Up-Forward
        MATRIX mainRot = Transform<D>.Slerp(Transform<D>.PlaneRotation(yz, 1, 2), Transform<D>.PlaneRotation(90.0f, 1, 3), volumeSmooth);

        //Combine with secondary rotation
        return gravityMatrix * Transform<D>.SkipY(m1Rot) * mainRot;
    }
