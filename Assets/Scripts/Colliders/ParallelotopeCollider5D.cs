...
public class ParallelotopeCollider5D : Collider5D {
    public Vector5 pos = Vector5.zero;
    public Vector5 ax1 = (Vector5)new Vector3(1, 0, 0);
    public Vector5 ax2 = (Vector5)new Vector3(0, 1, 0);
    public Vector5 ax3 = (Vector5)new Vector3(0, 0, 1);
#if USE_5D
    public Vector5 ax4 = (Vector5)new Vector4(0, 0, 0, 1);
#endif
    ...
    public override Vector5 NP(Vector5 localPt) {
        ...
        Vector5 p = localPt - pos;

        //Since this is flat right now, just project to the space
#if USE_5D
        Vector5 ax5 = Transform5D.MakeNormal(ax1, ax2, ax3, ax4);
        p -= Vector5.Project(p, ax5);
        Vector5 np = NP4(p, ax1, ax2, ax3, ax4);
#else
        Vector5 ax4 = Transform5D.MakeNormal(ax1, ax2, ax3);
        p -= Vector5.Project(p, ax4);
        Vector5 np = NP3(p, ax1, ax2, ax3);
#endif
    ...
    public static Vector5 NP4(Vector5 p, Vector5 ax1, Vector5 ax2, Vector5 ax3, Vector5 ax4) {
        //Get new cofactors
        Vector5 cf1 = ax1 - Transform5D.Project(ax1, ax2, ax3, ax4);
        Vector5 cf2 = ax2 - Transform5D.Project(ax2, ax3, ax4, ax1);
        Vector5 cf3 = ax3 - Transform5D.Project(ax3, ax4, ax1, ax2);
        Vector5 cf4 = ax4 - Transform5D.Project(ax4, ax3, ax1, ax2);

        //Calculate projection factors
        float dp1 = Vector5.Dot(cf1, p);
        float dp2 = Vector5.Dot(cf2, p);
        float dp3 = Vector5.Dot(cf3, p);
        float dp4 = Vector5.Dot(cf4, p);
        ...
        Vector5 p1 = p - ax1; p1 -= Vector5.Project(p1, cf1);
        Vector5 p2 = p - ax2; p2 -= Vector5.Project(p2, cf2);
        Vector5 p3 = p - ax3; p3 -= Vector5.Project(p3, cf3);
        Vector5 p4 = p - ax4; p4 -= Vector5.Project(p4, cf4);

        //Calculate nearest point on each parallelepiped
        Vector5 np1 = ax1 + NP3(p1, ax2, ax3, ax4);
        Vector5 np2 = ax2 + NP3(p2, ax3, ax4, ax1);
        Vector5 np3 = ax3 + NP3(p3, ax4, ax1, ax2);
        Vector5 np4 = ax4 + NP3(p4, ax1, ax2, ax3);
    ...
    //NOTE: p must already be on the space
    public static Vector5 NP3(Vector5 p, Vector5 ax1, Vector5 ax2, Vector5 ax3) {
        //Get new cofactors
        Vector5 cf1 = ax1 - Transform5D.Project(ax1, ax2, ax3);
        Vector5 cf2 = ax2 - Transform5D.Project(ax2, ax3, ax1);
        Vector5 cf3 = ax3 - Transform5D.Project(ax3, ax1, ax2);
        ...
        Vector5 np1 = ax1 + NP2(p1, ax2, ax3);
        Vector5 np2 = ax2 + NP2(p2, ax3, ax1);
        Vector5 np3 = ax3 + NP2(p3, ax1, ax2);
    ...
    public static Vector5 NP2(Vector5 p, Vector5 ax1, Vector5 ax2) {
        //Get new cofactors
        Vector5 cf1 = ax1 - Transform5D.Project(ax1, ax2);
        Vector5 cf2 = ax2 - Transform5D.Project(ax2, ax1);
        ...
        Vector5 np1 = ax1 + NP1(p1, ax2);
        Vector5 np2 = ax2 + NP1(p2, ax1);
    ...
    public static Vector5 NP1(Vector5 p, Vector5 ax1) {
        return ax1 * Mathf.Clamp(Vector5.Dot(p, ax1) / ax1.sqrMagnitude, -1.0f, 1.0f);
    }

    public static bool IsParallelotope(Vector5 a1, Vector5 a2, Vector5 A1, Vector5 A2, Vector5 b1, Vector5 b2, Vector5 B1, Vector5 B2) {
    ...
    public static bool IsParallelotope(Vector5 wa1, Vector5 wa2, Vector5 wA1, Vector5 wA2, Vector5 wb1, Vector5 wb2, Vector5 wB1, Vector5 wB2,
