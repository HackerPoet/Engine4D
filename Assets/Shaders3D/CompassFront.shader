    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(v2f i) : SV_Target {
                ...
                // sample the texture
                float3 lightDir = float3(0.4, 0.9, 0.3);
                float3 n = normalize(i.normal);
                float lightDot = dot(n, lightDir);
                float specular = pow(max(0.0, lightDir.z - (2.0 * lightDot) * n.z), 12.0);
                return float4(1.0, 1.0, 1.0, specular * 0.5);
          }
          ENDCG
      }
    }
